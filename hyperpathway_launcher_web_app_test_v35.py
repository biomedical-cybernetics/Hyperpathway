...
def add_download_buttons(fig, key_suffix, filename):
    """Add download buttons for high-res exports."""
    ...
    with cols[0]:
        try:
            ...
            st.download_button(
                ...
                mime="image/svg+xml",
                key=f"svg_{key_suffix}",
                help="Vector format - infinite resolution"
            )
        except:
    ...
    with cols[1]:
        try:
            ...
            st.download_button(
                ...
                mime="image/png",
                key=f"png_{key_suffix}",
                help="3000x3000px @ 300 DPI"
            )
        except:
            st.warning("Install kaleido: pip install kaleido")
...
<div style="display: flex; justify-content: flex-end; gap: 15px; align-items: center; margin-bottom: 10px;">
    <a href="https://brain.tsinghua.edu.cn/en/Research1/Research_Centers/Complex_Network_Intelligence_Center.htm" target="_blank">
        <img src="https://img.icons8.com/ios-filled/50/000000/home.png" width="40" title="Visit CCNI Lab"/>
    </a>
    <a href="https://github.com/biomedical-cybernetics" target="_blank">
        <img src="https://img.icons8.com/ios-glyphs/30/000000/github.png" width="40" title="Visit our GitHub"/>
    </a>
</div>
...
with col1:
    st.image("black_hyperbolic_sphere.png", width=150)

with col2:
    st.markdown("""
        <h1 style='margin-bottom: 5px; padding-top: 50px;'>Hyperpathway Web App</h1>
        <div style='font-size:18px; color: gray; margin-top: 5px;'>
            Contact: <a href="mailto:ilyes.abdelhamid1@gmail.com">ilyes.abdelhamid1@gmail.com</a><br>
            For more information on our lab and work, visit the <b>Center of Complex Network Intelligence (CCNI)</b> in <b>Tsinghua</b> university (Beijing, China) and our <b>GitHub</b> page by clicking the icons in the top right corner.
        </div>
    """, unsafe_allow_html=True)
...
st.markdown("""
<div style='text-align: center; font-size:18px; color: gray; margin-bottom: 25px;'>
    Based on the method described in:<br>
    <b>Ilyes Abdelhamid</b> et al. <i><a href='https://doi.org/...' target='_blank'>Hyperpathway: visualizing organization of pathway-molecule enriched interactions in omics studies via hyperbolic network embedding.</a></i> XXX, 2025.
</div>
""", unsafe_allow_html=True)
...
st.markdown(
    ...
    <div style='font-size:20px; font-weight: bold; margin-bottom: -2px; line-height: 1.2;'>
        🔬 Select the type of omics data you want to visualize:
    </div>
    """,
    unsafe_allow_html=True
)
...
## Selection of -omic type to visualize
omics_type = st.selectbox(
    "Select omics type",
    ["-- Select omics type --", "Lipidomics", "Genomics", "Metabolomics", "Others"],
    key='selection_omics',
    label_visibility="collapsed",
)

if "previous_omics_type" in st.session_state and st.session_state["previous_omics_type"] != omics_type:
...
st.session_state["previous_omics_type"] = omics_type

# Block further actions until a valid choice is made
if omics_type == "-- Select omics type --":
    st.warning("⚠️ Please select an omics type to proceed.")
    st.stop()
# Proceed based on the selected type
st.markdown(f"**You selected:** `{omics_type}`")

if omics_type == "Lipidomics":
    # Show lipid-specific options
    ...
    st.warning("‘Other’ selected — please make sure your input file format is supported.")
...
<div style='border: 2px solid #FF4B4B; padding: 15px; border-radius: 10px; background-color: #FFF8F8;'>
    ...
    <h4 style='color: #C70039; margin-top: 0;'>📂 Demo files are available for download to guide you through the correct format.</h4>
</div>
...
with st.expander("▼ Expand to see file format and coloring guide"):
    ...
    st.image("example_file_format.png", width=800)
...
# --- Row 3: Lipidomics ---
    ...
    st.image("example_option2_file_format.png", width=600)
    ...
    """
        <div style='font-size:18px;'>
            ...
            <ul>
                <li><b>Similarity</b>: Angular coordinate-based gradient coloring. Pathway nodes with similar network positions receive similar hues (HSV gradient). Molecule nodes are colored as the average of their connected pathway colors. Reflects topological similarity in hyperbolic space.</li>
                <li><b>Hierarchy</b>: Radial coordinate-based coloring. Blue-to-red gradient based on node degree centrality (low-degree = blue, high-degree = red). Molecule nodes are colored as the average of their connected pathway colors.</li>
                <li><b>Labels</b>: Category-based coloring using pathway annotations (e.g., pathway class, functional community). Pathways sharing the same label receive identical colors from a maximally distinct HSV palette. Molecule nodes are colored as the average of their connected pathway colors.</li>
                <li><b>Preference</b>: Upload your own colors for individual nodes. Supports HEX (#FF0000), space-separated RGB (255 0 0), or normalized RGB (1 0 0).</li>
                <li><b>Default</b> (for option 2 only): Uses custom colors from node file. Nodes without specified colors appear black (diamond node) or red (circle node).</li>   
        </div>
        <div style='font-size:18px;'>
            <b>🎨 Edge Coloring</b>:<br>
            <ul>
                <li><b>Similarity/Hierarchy/Labels modes</b>: Edges inherit the color of their connected pathway node.</li>
...
st.markdown(
    """
    <div style='font-size:20px; font-weight: bold; margin-bottom: -2px; line-height: 1.2;'>
        Upload your pathway enrichment results table.
    </div>
    """,
    unsafe_allow_html=True
)
# --- FIRST uploader: for pathway enrichment table ---
...
# Display a big centered OR separator
st.markdown("""
<div style='text-align: center; font-size: 40px; font-weight: bold; color: #888;'>OR</div>
""", unsafe_allow_html=True)

# --- SECOND uploader: for bipartite network ---
st.markdown(
    """
    <div style='font-size:20px; font-weight: bold; margin-bottom: -2px; line-height: 1.2;'>
        Upload your bipartite adjacency list file.
    </div>
    """,
    unsafe_allow_html=True
)
...
if uploaded_bipartite_file is not None:
    current_file_id = f"{uploaded_bipartite_file.name}_{uploaded_bipartite_file.size}"
    if st.session_state.get("last_bipartite_file_id") != current_file_id:
        # New file detected - clear previous visualizations
        ...
        for key in keys_to_clear:
            if key in st.session_state:
                del st.session_state[key]
        st.session_state["last_bipartite_file_id"] = current_file_id
else:
    # File was removed - clear all visualizations
    if st.session_state.get("last_bipartite_file_id") is not None:
        keys_to_clear = ["fig1", "coords_embedding_fig1", "excel_buffer_fig1", "selected_nodes_fig1",
                         "fig2", "option2_coords_fig2", "excel_buffer_fig2", "selected_nodes_option2_fig2",
                         "wcolor_base_option2_fig1", "wcolor_base_option2_fig2", "last_bipartite_file_id"]
        for key in keys_to_clear:
            if key in st.session_state:
                del st.session_state[key]
...
if uploaded_pea_file and uploaded_bipartite_file:
        ...
        with col1:
            ...
            if st.session_state.selected_nodes_fig1:
                ...
                mask, coords_sub, colors_sub, names_sub, shapes_sub, edge_colors_sub = filter_subgraph(
                    ...
                    fixed_names, wsymbol, st.session_state.selected_nodes_fig1,
        ...
        with col2:
            ...
            if st.session_state.selected_nodes_fig2:
                ...
                mask, coords_sub, colors_sub, names_sub, shapes_sub, edge_colors_sub = filter_subgraph(
                    ...
                    fixed_names, wsymbol, st.session_state.selected_nodes_fig2,
                    edge_colors=edge_colors if 'edge_colors' in locals() else None 
                )
            ...
            if "option2_fig1" in st.session_state:
                ...
                if st.session_state.selected_nodes_option2_fig1:
                    ...
                    mask, coords_sub, colors_sub, names_sub, shapes_sub, edge_colors_sub = filter_subgraph(
                        ...
                        fixed_names, wsymbol, st.session_state.selected_nodes_option2_fig1,
                ...
                if st.session_state.selected_nodes_option2_fig2:         
                    try:
                        ...
                        mask, coords_sub, colors_sub, names_sub, shapes_sub, edge_colors_sub = filter_subgraph(
                            ...
                            fixed_names, wsymbol, st.session_state.selected_nodes_option2_fig2,
                            edge_colors=edge_colors_to_pass
                        )
